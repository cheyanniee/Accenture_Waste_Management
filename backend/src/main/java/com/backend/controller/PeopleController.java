...
public class PeopleController {
    ...
    //checking if the lists worked - district and region
    @PostMapping("checkadd")
    public void checkAdd(@RequestBody PeopleRequest peopleRequest) {
        createLists();
        String districtCode ="";
        String regionName = "";
            ...
            //get the district
            for (String postalCode : district.keySet()){
                List<String> values = district.get(postalCode);
                for(String digits : values){
                    if (digits.equals(first2digits)){
            ...
            //get the region
            for (String regionN : region.keySet()){
                List<String> values = region.get(regionN);
                for (String districtC : values){
                    if (districtC.equals(districtCode)){
                        regionName = regionN;
                    }
                }
            }
            System.out.println("The district is: " + districtCode);
            System.out.print("The region is: " + regionName);
    }
